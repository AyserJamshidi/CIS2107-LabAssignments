 ...
 */
int all_letters(char *s); // 5 points

/*
 * Returns the number of characters c in s1 such that b<=c<=t
 */
int num_in_range(char *s1, char b, char t); // 5 points
 ...
 */
int strcmp_ign_case(char *s1, char *s2); // 8 points
 ...
 */
void take_last(char *s, int n); // 6 points
 ...
 */
char *dedup(char *s); // 10 points

/*
 * Returns a new string consisting of all of the letters of s, but
 * padded with spaces at the end so that the total length of the
 * returned string is an even multiple of d. If the length of s is
 * already an even multiple of d, the function returns a copy of s.
 *
 ...
 */
char *pad(char *s, int d); // 10 points

/*
 * Returns 1 if suff is a suffix of s ignoring case or 0 otherwise.
 */
int ends_with_ignore_case(char *s, char *suff); // 10 points
 ...
 * It is up to the caller to free any memory allocated by the function.
 */
char *repeat(char *s, int x, char sep); // 10 points
 ...
 */
char *str_connect(char **strs, int n, char c); // 10 points

/*
 * Words is an array of string terminated with a NULL pointer. The
 * function removes any empty strings (i.e., strings of length 0)
 * from the array.
 */
void rm_empties (char **words); // 10 points
 ...
 */
char **str_chop_all (char *s, char c); // 10 points